...
export default function EditRawMaterial({ params }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: item }, { data: cats }, { data: sups }, { data: whs }] = await Promise.all([
        ...
        supabase.from("raw_materials_warehouses").select("warehouse").eq("raw_material_id", id),
      ]);
      setForm(item);
  ...
  async function update() {
    ...
    if (warehouses.length > 0) {
      ...
      if (wErr) { alert("Material updated but warehouse save failed: " + wErr.message); return; }
    }
