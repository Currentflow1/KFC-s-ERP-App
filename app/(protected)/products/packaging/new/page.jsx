...
export default function NewPackaging() {
  ...
  async function submit() {
    ...
    if (warehouses.length > 0) {
      const whRows = warehouses.map((w) => ({ packaging_id: inserted.id, warehouse: w }));
      const { error: wErr } = await supabase.from("packaging_warehouses").insert(whRows);
      if (wErr) { alert("Packaging saved but warehouse save failed: " + wErr.message); return; }
    }

    // packaging_inventory.id has no DB default — must generate it ourselves.
    // One row per warehouse, or a single warehouse-less row if none were added.
    const invRows = (warehouses.length > 0 ? warehouses : [null]).map((w) => ({
      id: crypto.randomUUID(),
      name: form.name,
      packaging_id: inserted.id,
      warehouse: w,
    }));
    const { error: invErr } = await supabase.from("packaging_inventory").insert(invRows);
    if (invErr) { alert("Packaging saved but inventory row failed: " + invErr.message); return; }
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen flex justify-center">
      <div className="bg-white border rounded-xl shadow-sm p-6 w-full max-w-2xl">
        <h1 className="text-black text-xl font-bold mb-6">New Packaging</h1>
        <div className="space-y-4">

          <input name="name" placeholder="Packaging Name" value={form.name} onChange={handleChange}
