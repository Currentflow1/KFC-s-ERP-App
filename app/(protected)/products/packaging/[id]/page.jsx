        ...
        <div className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg">
          <div className="p-2 border-b border-gray-100">
            <input autoFocus type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search…"
              className="text-black w-full text-sm px-2 py-1.5 border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-blue-500" />
          </div>
...
export default function EditPackaging({ params }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: item }, { data: cats }, { data: sups }, { data: whs }] = await Promise.all([
        supabase.from("packaging_static").select("*").eq("id", id).single(),
        supabase.from("categories").select("name"),
        supabase.from("suppliers").select("contact_person"),
        supabase.from("packaging_warehouses").select("warehouse").eq("packaging_id", id),
      ]);
      setForm(item);
      setCategories((cats || []).map((c) => c.name));
      setSuppliers((sups || []).map((s) => s.contact_person));
      setWarehouses((whs || []).map((w) => w.warehouse));
    }
    load();
  }, [id]);
  ...
  async function update() {
    ...
    await supabase.from("packaging_warehouses").delete().eq("packaging_id", id);
    if (warehouses.length > 0) {
      const rows = warehouses.map((w) => ({ packaging_id: id, warehouse: w }));
      const { error: wErr } = await supabase.from("packaging_warehouses").insert(rows);
      if (wErr) { alert("Packaging updated but warehouse save failed: " + wErr.message); return; }
    }

    // Make sure every current warehouse has a matching packaging_inventory row.
    // Existing warehouses already have one; only newly added ones are missing it.
    const { data: existingInv } = await supabase
      .from("packaging_inventory")
      .select("warehouse")
      .eq("packaging_id", id);
    const existingWarehouses = new Set((existingInv || []).map((r) => r.warehouse));

    const missing = (warehouses.length > 0 ? warehouses : [null]).filter(
      (w) => !existingWarehouses.has(w)
    );
    if (missing.length > 0) {
      const newInvRows = missing.map((w) => ({
        id: crypto.randomUUID(),
        name: form.name,
        packaging_id: id,
        warehouse: w,
      }));
      const { error: invErr } = await supabase.from("packaging_inventory").insert(newInvRows);
      if (invErr) { alert("Packaging updated but inventory row failed: " + invErr.message); return; }
    }
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen flex justify-center">
      <div className="w-full max-w-2xl bg-white border rounded-xl shadow-sm p-6">
        <h1 className="text-black text-xl font-bold mb-6">Edit Packaging</h1>
        <div className="space-y-4">

          <input name="name" value={form.name || ""} onChange={handleChange} placeholder="Packaging Name"
          ...
          <label className="text-black flex items-center space-x-2 text-sm">
            <input type="checkbox" name="discontinued" checked={form.discontinued ?? false} onChange={handleChange} />
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button onClick={update} className="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700">Update</button>
          </div>
        </div>
      </div>
    </div>
