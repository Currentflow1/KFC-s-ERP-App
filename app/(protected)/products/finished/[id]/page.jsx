...
export default function EditFinishedProduct({ params }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: product }, { data: cats }, { data: whs }] = await Promise.all([
        supabase.from("finished_products_static").select("*").eq("id", id).single(),
        supabase.from("categories").select("name"),
        supabase.from("finished_products_warehouses").select("warehouse").eq("finished_product_id", id),
      ]);
      setForm(product);
  ...
  async function update() {
    ...
    const { error } = await supabase
      ...
      .update({ name: form.name, category_name: form.category_name, discontinued: form.discontinued })
      .eq("id", id);
    ...
    if (warehouses.length > 0) {
      ...
      const { error: wErr } = await supabase.from("finished_products_warehouses").insert(rows);
      if (wErr) { alert("Product updated but warehouse save failed: " + wErr.message); return; }
    }
