...
function StaffTable() {
  ...
  async function save() {
    ...
    if (editRow) {
      ...
      const { error: e } = await supabase.from("staff_employee").insert(payload);
      if (e) { setError(e.message); setSaving(false); return; }
      setShowAdd(false);
    }
    setSaving(false); load();
  ...
  async function confirmDelete() {
    const supabase = createClient();
    await supabase.from("staff_employee").delete().eq("id", deleteRow.id);
    setDeleteRow(null); load();
  }
    ...
    <div className="bg-white text-gray-900 border border-gray-200 rounded-lg shadow-sm overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-gray-400 text-[10px]">{open ? "▲" : "▼"}</span>
          <div>
            <h2 className="text-sm font-semibold text-gray-900">Staff Employees</h2>
            <p className="text-xs text-gray-400 mt-0.5">{rows.length} employee{rows.length !== 1 ? "s" : ""}</p>
          </div>
        </div>
        <div onClick={(e) => e.stopPropagation()}>
      ...
      {deleteRow && (
        <ConfirmModal
          message={`Delete "${deleteRow.name || "this staff member"}"? This cannot be undone.`}
          onConfirm={confirmDelete}
          onCancel={() => setDeleteRow(null)}
        />
      )}
    ...
    <div className="px-6 py-5 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-start mb-5">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage monitoring, representative, and staff personnel</p>
        </div>
      </div>
