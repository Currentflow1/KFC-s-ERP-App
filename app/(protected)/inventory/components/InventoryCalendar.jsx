...
export default function InventoryCalendar({ tab, date, onSelectDate }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        className="border px-4 py-2 rounded bg-white text-gray-900 min-w-[150px] text-left"
      >
        {date ? `📅 ${date}` : "📅 Today (live)"}
      </button>

      {open && (
        <div className="absolute z-10 mt-2 bg-white border rounded shadow-lg p-3 w-72 text-gray-900">
          <div className="flex items-center justify-between mb-2">
            <button onClick={() => changeMonth(-1)} className="px-2 py-1 border rounded text-sm text-gray-700 hover:bg-gray-100">‹</button>
                ...
                <button
                  ...
                  className={`relative py-1 rounded ${
                    isSelected
                      ...
                      : isToday
                        ? "border border-blue-600 text-gray-900"
