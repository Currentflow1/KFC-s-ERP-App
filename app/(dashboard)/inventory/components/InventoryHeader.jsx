...
export default function InventoryHeader({ onPrint, items = [], date = "" }) {
  ...
  function handlePrint() {
    ...
    out += divider + "\n";
    out += cols.map((c) => (c.key === "name" ? pad(c.label, c.width) : padNum(c.label, c.width))).join("") + "\n";
    out += divider + "\n";
    items.forEach((item) => {
      out += cols.map((c) => (c.key === "name" ? pad(item.name, c.width) : padNum(item[c.key], c.width))).join("") + "\n";
    });
    out += divider + "\n";
  ...
  return (
    <div className="mb-6 flex justify-between items-center">
      <div>
        ...
        <p className="text-sm text-gray-500">Manage inventory system</p>
      </div>
      <div className="flex gap-2">
        <button onClick={() => exportInventoryCSV(items, date)} className="bg-green-600 text-white px-4 py-2 rounded">Export CSV</button>
        <button onClick={handlePrint} className="bg-black text-white px-4 py-2 rounded">Print</button>
      </div>
    </div>
  );
}