...
async function _flushInventory(supabase) {
  ...
  for (const row of rows ?? []) {
    try {
      ...
      if (row.table_name === "finished_products_inventory" && !payload.finished_product_id) {
        await markFailed(row.id, row.table_name, "missing finished_product_id");
        continue;
      }
      if (row.table_name === "raw_materials_inventory" && !payload.raw_material_id) {
        await markFailed(row.id, row.table_name, "missing raw_material_id");
        continue;
      }
...
export function sanitizeInventoryRow(row) {
  ...
  return {
    ...
    actual_bal,
    loss,
    // Always emit both FK columns unconditionally so the upsert never
    // violates the NOT NULL constraint. If a value is genuinely absent it
    // will surface as a Supabase error (caught by markFailed) rather than
    // silently writing a broken row or producing a 400 with no context.
...
export async function evictStaleQueueEntries() {
  try {
    ...
    for (const row of rows ?? []) {
      try {
        const payload = JSON.parse(row.payload);
        const needsFinFk = row.table_name === "finished_products_inventory" && !payload.finished_product_id;
        const needsRawFk = row.table_name === "raw_materials_inventory"     && !payload.raw_material_id;
        if (needsFinFk || needsRawFk) {
          const missingField = needsFinFk ? "finished_product_id" : "raw_material_id";
          await markFailed(row.id, row.table_name, `evicted on boot — missing ${missingField}`);
        }
      } catch (e) {
    ...
    // Those snapshot rows may be missing the FK too, so undoing them would
    // produce a 400. Clearing forces a fresh snapshot on the next change.
    await db.item_change_history.clear();
