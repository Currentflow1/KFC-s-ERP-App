...
// FIX: this previously returned `true` as soon as the browser reported
// itself online, without checking whether the row it just queued actually
// made it into Supabase. flushQueue() can silently leave a row unsynced
// (transient network error, RLS rejection, etc.) or mark it SYNC_FAILED
// (schema mismatch / missing FK). Callers like ManipulatePanel.applyChange
// trust this return value to decide whether to also write the transaction
// log directly to Supabase (online path) or queue it locally alongside the
// inventory change (offline path). A false-positive `true` here caused the
// tx log to be written straight to Supabase even though the matching
// inventory upsert never landed — producing tx-log rows with no matching
// inventory update, and a stale pending_changes row left behind to be
// retried later and clobber whatever the row had become in the meantime.
//
// Now: after flushing, re-read this specific row from IndexedDB and only
// report success if it actually reached SYNC_SYNCED.
export async function writeInventory(tableName, recordId, payload) {
  const localId = await db.pending_changes.add({
  ...
  try {
    const row = await db.pending_changes.get(localId);
    return row?.synced === SYNC_SYNCED;
  } catch (e) {
    console.error(`[sync] writeInventory verify id=${localId}:`, e);
    return false;
  }
}
